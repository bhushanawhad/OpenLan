function HandleSigninPage(json) {
	$(document).ready(function () { ShowSigninPage(json) });
}

var identityProviders = [];
var cookieName = "ACSChosenIdentityProvider-10000015";
	...
	if (img.height > maxImageHeight || img.width > maxImageWidth) {
		...
		if (img.height > 0) {
			if (img.width / img.height > maxImageWidth / maxImageHeight) {
				...
				resizeRatio = maxImageWidth / (img.width || 1);
			}
			else {
